...
function AnimatedFigure({
  ...
  return (
    <Animated.View style={[styles.figureContainer, { top, left, width, height }, style]}>
      <Image
        source={require('@/assets/images/figurewel2.png')}
        style={styles.figure}
        contentFit="contain"
      />
    </Animated.View>
  ...
  return (
    <View style={[styles.welcomeText, { left: (width - textWidth) / 2, width: textWidth, top }]}>
      <Animated.Text style={[styles.title, { fontSize: titleSize, lineHeight: titleSize * 1.47 }, titleStyle]}>
        Remote Control & Secure Access
      </Animated.Text>
      <Animated.Text style={[styles.description, { fontSize: descSize, lineHeight: descSize * 1.43 }, descStyle]}>
        Lock and unlock remotely, validate PINs, and receive instant alerts for unauthorized access.
      </Animated.Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  barOverlay: {
    position: 'absolute',
    left: 17, // Centraliza a barra (64px pontos - 30px barra) / 2 = 17px
    top: 9.5,
